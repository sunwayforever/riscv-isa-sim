...
// The Handcar simulator is to be loaded dynamically. All function addresses can
// then be set...
...
struct SimDllApi {
    ...
    // int (*set_simulator_parameter)(const char*, const uint64_t*, const
    // char*);
    ...
    // int (*partial_read_large_register)(int, const char*, uint8_t*, uint32_t,
    // uint32_t);
    // int (*partial_write_large_register)(
    //     int, const char *, const uint8_t *, uint32_t, uint32_t);
    ...
    // int (*translate_virtual_address)(int,
    // const uint64_t*, int, uint64_t*, uint64_t*); int
    // (*initialize_simulator_memory)(int, const uint64_t*, int, uint64_t);

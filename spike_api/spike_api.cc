...
static void help(int exit_code = 1) {
    ...
    fprintf(
        stderr,
        "  -m<n>                 Provide <n> MiB of target memory [default "
        "2048]\n");
    fprintf(
        stderr,
        "  -m<a:m,b:n,...>       Provide memory regions of size m and n "
        "bytes\n");
    fprintf(
        stderr,
        "                          at base addresses a and b (with 4 KiB "
        "alignment)\n");
    ...
    fprintf(
        stderr,
        "  -H                    Start halted, allowing a debugger to "
        "connect\n");
    ...
    fprintf(
        stderr, "  --isa=<name>          RISC-V ISA string [default %s]\n",
        DEFAULT_ISA);
    fprintf(
        stderr, "  --pmpregions=<n>      Number of PMP regions [default 16]\n");
    fprintf(
        stderr,
        "  --priv=<m|mu|msu>     RISC-V privilege modes supported [default "
        "%s]\n",
        DEFAULT_PRIV);
    fprintf(
        stderr,
        "  --varch=<name>        RISC-V Vector uArch string [default %s]\n",
        DEFAULT_VARCH);
    fprintf(stderr, "  --pc=<address>        Override ELF entry point\n");
    fprintf(
        stderr,
        "  --hartids=<a,b,...>   Explicitly specify hartids, default is "
        "0,1,...\n");
    fprintf(
        stderr,
    ...
    fprintf(
        stderr, "  --big-endian          Use a big-endian memory system.\n");
    fprintf(
        stderr, "  --misaligned          Support misaligned memory accesses\n");
    fprintf(
        stderr,
        "  --device=<P,B,A>      Attach MMIO plugin device from an --extlib "
        "library\n");
    ...
    fprintf(
        stderr,
        "                          A -- String arguments to pass to the "
        "plugin\n");
    fprintf(
        stderr,
    ...
    fprintf(
        stderr,
        "                          The extlib flag for the library must come "
        "first.\n");
    ...
    fprintf(
        stderr,
        "  --rbb-port=<port>     Listen on <port> for remote bitbang "
        "connection\n");
    fprintf(
        stderr, "  --dump-dts            Print device tree string and exit\n");
    fprintf(
        stderr,
        "  --dtb=<path>          Use specified device tree blob [default: "
        "auto-generate]\n");
    fprintf(
        stderr,
        "  --disable-dtb         Don't write the device tree blob into "
        "memory\n");
    fprintf(
        stderr, "  --kernel=<path>       Load kernel flat image into memory\n");
    fprintf(stderr, "  --initrd=<path>       Load kernel initrd into memory\n");
    fprintf(
        stderr,
        "  --bootargs=<args>     Provide custom bootargs for kernel [default: "
        "%s]\n",
    ...
    fprintf(
        stderr,
        "  --dm-sba=<bits>       Debug system bus access supports up to "
        "<bits> wide accesses [default 0]\n");
    fprintf(
        stderr,
        "  --dm-auth             Debug module requires debugger to "
        "authenticate\n");
    fprintf(
        stderr,
        "  --dmi-rti=<n>         Number of Run-Test/Idle cycles "
        "required for a DMI access [default 0]\n");
    fprintf(
        stderr,
        "  --dm-abstract-rti=<n> Number of Run-Test/Idle cycles "
        "required for an abstract command to execute [default 0]\n");
    fprintf(stderr, "  --dm-no-hasel         Debug module supports hasel\n");
    fprintf(
        stderr,
        "  --dm-no-abstract-csr  Debug module won't support abstract CSR "
        "access\n");
    fprintf(
        stderr,
        "  --dm-no-abstract-fpr  Debug module won't support abstract FPR "
        "access\n");
    fprintf(
        stderr,
    ...
    fprintf(
        stderr,
        "  --dm-no-impebreak     Debug module won't support implicit ebreak in "
        "program buffer\n");
    fprintf(
        stderr,
        "  --blocksz=<size>      Cache block size (B) for CMO "
        "operations(powers of 2) [default 64]\n");
...
static void read_file_bytes(
    const char* filename, size_t fileoff, mem_t* mem, size_t memoff,
    size_t read_sz) {
...
static bool check_mem_overlap(const mem_cfg_t& L, const mem_cfg_t& R) {
    return std::max(L.get_base(), R.get_base()) <=
           std::min(L.get_inclusive_end(), R.get_inclusive_end());
}

static bool check_if_merge_covers_64bit_space(
    const mem_cfg_t& L, const mem_cfg_t& R) {
    if (!check_mem_overlap(L, R)) return false;
...
static std::vector<mem_cfg_t> merge_overlapping_memory_regions(
    ...
    for (auto mem_it = std::next(mems.begin()); mem_it != mems.end();
         ++mem_it) {
...
static std::vector<mem_cfg_t> parse_mem_layout(const char* arg) {
    ...
    while (true) {
        ...
        const unsigned long long max_allowed_pa =
            (1ull << MAX_PADDR_BITS) - 1ull;
...
static std::vector<std::pair<reg_t, mem_t*>> make_mems(
    const std::vector<mem_cfg_t>& layout) {
    std::vector<std::pair<reg_t, mem_t*>> mems;
    mems.reserve(layout.size());
    for (const auto& cfg : layout) {
        mems.push_back(
            std::make_pair(cfg.get_base(), new mem_t(cfg.get_size())));
    }
    return mems;
}
...
static reg_t get_csr(int target, const char* regname) {
...
extern "C" int read_simulator_register(
    int target, const char* name, uint8_t* buffer, int len) {
    processor_t* p = g_sim->get_core(target);

    if (strcmp(name, "pc") == 0) {
        reg_t value = p->get_state()->pc;
    ...
    if (r == NXPR) {
        ...
        reg = p->get_state()->XPR[r];
    }
    memcpy(buffer, &reg, len);
    return 0;
}

extern "C" int write_simulator_register(
    int target, const char* name, uint8_t* buffer, int len) {
    processor_t* p = g_sim->get_core(target);
    if (strcmp(name, "pc") == 0) {
    ...
    unsigned long r =
        std::find(xpr_name, xpr_name + NXPR, std::string(name)) - xpr_name;
    if (r == NXPR) {
        put_csr(target, name, reg);
        return 0;
    } else {
        p->get_state()->XPR.write(r, reg);
    }
    return 0;
}

extern "C" int step_simulator(int target, int n, int fail) {
    processor_t* p = g_sim->get_core(target);
    p->step(n);
    return 0;
}
...
extern "C" int initialize_simulator(int argc, char** args) {
    ...
    auto const device_parser = [&plugin_devices](const char* s) {
        ...
        // Parse the name, which is simply all of the characters leading up to
        // the first comma. The validity of the plugin name will be checked
        // later.
        std::string name;
        ...
        // Parse the base address. First, get all of the characters up to the
        // next comma (or up to the end of the string if there is no comma).
        // Then try to parse that string as an integer according to the rules of
        // strtoull. It could be in decimal, hex, or octal. Fail if we were able
        // to parse a number but there were garbage characters after the valid
        // number. We must consume the entire string between the commas.
        std::string base_str;
        ...
        // The remainder of the string is the arguments. We could use getline,
        // but that could ignore newline characters in the arguments. That
        // should be rare and discouraged, but handle it here anyway with this
        // weird in_avail technique. The arguments are optional, so if there
        // were no arguments specified we could end up with an empty string
        // here. That's okay.
...
#endif
    parser.option(
        'p', 0, 1, [&](const char* s) { nprocs = atoul_nonzero_safe(s); });
    ...
    parser.option(
        0, "pc", 1, [&](const char* s) { cfg.start_pc = strtoull(s, 0, 0); });
    ...
    parser.option(
        0, "dump-dts", 0, [&](const char UNUSED* s) { dump_dts = true; });
    ...
    parser.option(0, "dm-progsize", 1, [&](const char* s) {
        dm_config.progbufsize = atoul_safe(s);
    });
    parser.option(0, "dm-no-impebreak", 0, [&](const char UNUSED* s) {
        dm_config.support_impebreak = false;
    });
    parser.option(0, "dm-sba", 1, [&](const char* s) {
        dm_config.max_sba_data_width = atoul_safe(s);
    });
    parser.option(0, "dm-auth", 0, [&](const char UNUSED* s) {
        dm_config.require_authentication = true;
    });
    parser.option(
        0, "dmi-rti", 1, [&](const char* s) { dmi_rti = atoul_safe(s); });
    parser.option(0, "dm-abstract-rti", 1, [&](const char* s) {
        dm_config.abstract_rti = atoul_safe(s);
    });
    parser.option(0, "dm-no-hasel", 0, [&](const char UNUSED* s) {
        dm_config.support_hasel = false;
    });
    parser.option(0, "dm-no-abstract-csr", 0, [&](const char UNUSED* s) {
        dm_config.support_abstract_csr_access = false;
    });
    parser.option(0, "dm-no-abstract-fpr", 0, [&](const char UNUSED* s) {
        dm_config.support_abstract_fpr_access = false;
    });
    parser.option(0, "dm-no-halt-groups", 0, [&](const char UNUSED* s) {
        dm_config.support_haltgroups = false;
    });
    parser.option(
        0, "log-commits", 0, [&](const char UNUSED* s) { log_commits = true; });
    ...
    parser.option(0, "blocksz", 1, [&](const char* s) {
        ...
        if (blocksz < min_blocksz || blocksz > max_blocksz ||
            ((blocksz & (blocksz - 1))) != 0) {
            fprintf(
                stderr, "--blocksz must be a power of 2 between %u and %u\n",
    ...
    if (kernel && check_file_exists(kernel)) {
        ...
        for (auto& m : mems) {
            if (kernel_size &&
                (kernel_offset + kernel_size) < m.second->size()) {
                read_file_bytes(
                    kernel, 0, m.second, kernel_offset, kernel_size);
                break;
            }
        }
    }
    ...
    if (initrd && check_file_exists(initrd)) {
        ...
        for (auto& m : mems) {
            if (initrd_size && (initrd_size + 0x1000) < m.second->size()) {
                ...
                read_file_bytes(
                    initrd, 0, m.second, initrd_start - m.first, initrd_size);
                break;
            }
        }
    }
    ...
    g_sim = new sim_t(
        &cfg, halted, mems, plugin_devices, htif_args, dm_config, log_path,
        dtb_enabled, dtb_file, socket, cmd_file);
    std::unique_ptr<remote_bitbang_t> remote_bitbang((remote_bitbang_t*)NULL);
    std::unique_ptr<jtag_dtm_t> jtag_dtm(
        new jtag_dtm_t(&g_sim->debug_module, dmi_rti));
    if (use_rbb) {
        remote_bitbang.reset(new remote_bitbang_t(rbb_port, &(*jtag_dtm)));
        g_sim->set_remote_bitbang(&(*remote_bitbang));
    }

    if (dump_dts) {
        printf("%s", g_sim->get_dts());
        return 0;
    }
    ...
    for (size_t i = 0; i < cfg.nprocs(); i++) {
        if (ic) g_sim->get_core(i)->get_mmu()->register_memtracer(&*ic);
        if (dc) g_sim->get_core(i)->get_mmu()->register_memtracer(&*dc);
        for (auto e : extensions) g_sim->get_core(i)->register_extension(e());
        g_sim->get_core(i)->get_mmu()->set_cache_blocksz(blocksz);
    }
